...
public final class MainActivity extends Activity {
    ...
    private static final int APP_SURFACE = Color.rgb(246, 247, 245);
    private static final int SYSTEM_BAR = Color.rgb(21, 27, 25);
    ...
    private static final String EXACT_ALARM_PROMPTED = "exact_alarm_prompted_v913248";
    ...
    protected void onCreate(Bundle state) {
        ...
        root.setBackgroundColor(APP_SURFACE);
        webView = new WebView(this);
        webView.setLayoutParams(new FrameLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        webView.setBackgroundColor(APP_SURFACE);
    ...
    private void requestExactAlarmPermissionOnce() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S || ReminderScheduler.canScheduleExact(this)) {
            getSharedPreferences(RUNTIME_PREFS, MODE_PRIVATE).edit()
                    .putBoolean(EXACT_ALARM_PROMPTED, true).apply();
            requestOverlayPermissionOnce();
            return;
        }
        if (getSharedPreferences(RUNTIME_PREFS, MODE_PRIVATE)
